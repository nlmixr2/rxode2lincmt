test_that("solComp2 matches the known decomposition", {
...
test_that("solComp3 matches the known decomposition", {
  v <- list(L = c(5.8977832399092, 0.0122878774411843, 0.689928882649618),
            ...
            C3 = structure(c(-0.0798964923194813, 0.0489383595021832, 0.0296034459956658,
                           ...
                           dim = c(3L, 3L)))
  expect_equal(solComp3(k10 = 0.1, k12 = 3, k21 = 1, k13 = 2, k31 = 0.5), v)
})

test_that("solComp exponents are the eigenvalues of the rate matrix", {
  .s2 <- solComp2(k10 = 0.1, k12 = 3, k21 = 1)
  .K2 <- matrix(c(-(0.1 + 3), 3, 1, -1), 2)
  expect_equal(sort(.s2$L), sort(-eigen(.K2)$values))

  .s3 <- solComp3(k10 = 0.1, k12 = 3, k21 = 1, k13 = 2, k31 = 0.5)
  .K3 <- matrix(c(-(0.1 + 3 + 2), 3, 2, 1, -1, 0, 0.5, 0, -0.5), 3)
  expect_equal(sort(.s3$L), sort(-eigen(.K3)$values))
})

test_that("solComp rejects invalid rates", {
  expect_error(solComp2(k10 = -1, k12 = 3, k21 = 1))
  expect_error(solComp3(k10 = 0.1, k12 = 3, k21 = 1, k13 = NA, k31 = 0.5))
})
